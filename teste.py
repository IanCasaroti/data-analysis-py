...
if df.empty:
    ...
    with col6:
        # Ensure dates are in the index for correct plotting
        ...
        fig, ax = plt.subplots()
        ax.plot(data.index, data['last_daily_price'] * data['occupancy'])
    ...
    with col7:
        ...
        fig, ax = plt.subplots()
        ax.plot(data.index, data['last_daily_price'] * data['occupancy'] / data['last_daily_price'].sum() * 100)
        ax.yaxis.set_major_formatter('{x:1.2f}%')
        st.pyplot(fig)
    ...
    with col8:
        ...
        fig, ax = plt.subplots()
        ax.plot(data.index, data['last_daily_price'])
        ax.yaxis.set_major_formatter('${x:1.2f}')
        st.pyplot(fig)
